...
public class FilterBasedOnExtension : IntegrationTestBaseClass
{
    ...
    public void FilterOnMime_VIDEOFiltersSelectedFiles()
    {
        ...
        {
            Assert.Equal(file.ReadLine().Trim() , Path.Combine(PathToAssemblyFolder,testFolderPath,selectedTestItemsFiltered[0]).Trim());
            Assert.Equal(file.ReadLine().Trim() , Path.Combine(PathToAssemblyFolder,testFolderPath,selectedTestItemsFiltered[1]).Trim());
            Assert.Equal(file.ReadLine().Trim() , Path.Combine(PathToAssemblyFolder,testFolderPath,selectedTestItemsFiltered[2]).Trim());
