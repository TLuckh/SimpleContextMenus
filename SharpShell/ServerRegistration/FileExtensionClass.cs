        ...
        public static string Get(IRegistryKey classesKey, string fileExtension, bool createIfMissing)
        {
            ...
            if (string.IsNullOrEmpty(fileExtension) ||
                ...
                throw new InvalidOperationException(
                    $@"'{fileExtension}' does not appear to be a valid file extension class.");

            //  We will need the extension with no leading dot later.
            string extension = fileExtension.Substring(1);

            //  Open or create the file extension key.
            using (IRegistryKey fileExtensionClassKey =
                   classesKey.CreateSubKey(fileExtension, RegistryKeyPermissionCheck.ReadWriteSubTree))
            {
                ...
                string fileExtensionClassName = fileExtensionClassKey.GetValue(null) as string;
                ...
                using (IRegistryKey fileExtensionClassNameKey = classesKey.OpenSubKey(fileExtensionClassName))
                {
                    if (fileExtensionClassNameKey != null)
                    {
                        string applicationName = fileExtensionClassNameKey.GetValue(null) as string;
                        throw new InvalidOperationException(
                ...
                using (IRegistryKey fileExtensionClassNameKey = classesKey.CreateSubKey(fileExtensionClassName,
