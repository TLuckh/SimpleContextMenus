...
namespace SharpShell.SharpNamespaceExtension
{
...

    {
        ...
        {
            //  Use the ShellFolderImpl to handle the details.
            return ((IShellFolder)shellFolderImpl).GetUIObjectOf(hwndOwner, cidl, apidl, ref riid, rgfReserved,
                out ppv);
        }
        ...
        int IShellFolder2.GetUIObjectOf(IntPtr hwndOwner, uint cidl, IntPtr apidl, ref Guid riid, uint rgfReserved,
            out IntPtr ppv)
        {
            return ((IShellFolder2)shellFolderImpl).GetUIObjectOf(hwndOwner, cidl, apidl, ref riid, rgfReserved,
                out ppv);
        }
        ...
        /// </summary>
        /// <param name="pidl">
        ///     The address of the ITEMIDLIST (item identifier list) structure
        ///     that specifies the absolute location of the folder.
        /// </param>
        /// <returns>
        ///     If this method succeeds, it returns S_OK. Otherwise, it returns an HRESULT error code.
        /// </returns>
        ...
        internal static void CustomRegisterFunction(Type serverType, RegistrationType registrationType)
        {
            ...
            NamespaceExtensionJunctionPointAttribute junctionPoint =
                NamespaceExtensionJunctionPointAttribute.GetJunctionPoint(serverType);
            ...
            */
            ...
            RegistryHive hive = junctionPoint.Availablity == NamespaceExtensionAvailability.CurrentUser
                ? RegistryHive.CurrentUser
                : RegistryHive.LocalMachine;
            RegistryView view = registrationType == RegistrationType.OS64Bit
            ...
            using (RegistryKey baseKey = RegistryKey.OpenBaseKey(hive, view))
            {
                ...
                string virtualFolderNamespacePath =
                ...
                using (RegistryKey namespaceKey = baseKey.OpenSubKey(virtualFolderNamespacePath,
                           RegistryKeyPermissionCheck.ReadWriteSubTree, RegistryRights.WriteKey))
                {
                    ...
                    RegistryKey serverKey = namespaceKey.CreateSubKey(serverType.GUID.ToRegistryString());
                    if (serverKey == null)
                       ...
                       : RegistryKey.OpenBaseKey(RegistryHive.ClassesRoot, RegistryView.Registry32))
            {
                               ...
                               RegistryKeyPermissionCheck.ReadWriteSubTree, RegistryRights.WriteKey))
                {
                    ...
                    SharpNamespaceExtension serverInstance =
                        (SharpNamespaceExtension)Activator.CreateInstance(serverType);
                    NamespaceExtensionRegistrationSettings registrationSettings =
                        serverInstance.GetRegistrationSettings();
                    ...
                    if (registrationSettings.UseAssemblyIcon)
                        using (RegistryKey defaultIconKey = classKey.CreateSubKey(@"DefaultIcon"))
                        {
                            if (defaultIconKey == null)
                                throw new InvalidOperationException(
                                    "An exception occured creating the DefaultIcon key.");
                            defaultIconKey.SetValue(null, serverType.Assembly.Location, RegistryValueKind.String);
                        }

                    //  Set the attributes.
                    using (RegistryKey shellFolderKey = classKey.CreateSubKey("ShellFolder"))
                    {
                        if (shellFolderKey == null)
                            throw new InvalidOperationException("An exception occured creating the ShellFolder key.");
                        shellFolderKey.SetValue("Attributes", (int)registrationSettings.ExtensionAttributes,
