...
namespace SharpShell.SharpNamespaceExtension
{
    ...
    internal class ShellNamespaceFolderIdListEnumerator : IEnumIDList
    {
        ...
        public ShellNamespaceFolderIdListEnumerator(IShellNamespaceFolder shellNamespaceFolder, SHCONTF grfFlags,
            uint index)
        {
        ...
        /// <param name="rgelt">
        ///     Address of an array of ITEMIDLIST pointers that receives the item identifiers. The implementation must allocate
        ///     these item identifiers
        ///     using the Shell's allocator (retrieved by the SHGetMalloc function). The calling application is responsible for
        ///     freeing the item
        ///     identifiers using the Shell's allocator.
        /// </param>
        /// <param name="pceltFetched">
        ///     Address of a value that receives a count of the item identifiers actually returned in rgelt. The count can be
        ...
        public int Next(uint celt, IntPtr rgelt, out uint pceltFetched)
        {
            ...
            List<IShellNamespaceItem> items = new List<IShellNamespaceItem>();
            try
            {
            ...
            catch (Exception exception)
            {
                ...
                Logging.Error(
                    string.Format(
                        "An unhandled exception occured enumerating {0} items from the {1} namespace extension.", celt,
                        shellNamespaceFolder.GetDisplayName(DisplayNameContext.Normal)),
                    exception);
            }
            ...
            //  function is enough.
            IntPtr[] pidlArray = items
                .Select(iid => PidlManager.IdListToPidl(IdList.Create(new List<ShellId> { iid.GetShellId() })))
                .ToArray();
