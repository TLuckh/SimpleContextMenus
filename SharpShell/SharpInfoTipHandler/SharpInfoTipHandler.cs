...
namespace SharpShell.SharpInfoTipHandler
{
    ...
    public abstract class SharpInfoTipHandler : PersistFileServer, IQueryInfo
    {
        ...
        /// </summary>
        /// <param name="dwFlags">
        ///     Flags that direct the handling of the item from which you're retrieving the info tip text. This
        ///     value is commonly zero.
        /// </param>
        /// <param name="ppwszTip">
        ///     The address of a Unicode string pointer that, when this method returns successfully, receives the tip string
        ///     pointer. Applications that implement this method must allocate memory for ppwszTip by calling CoTaskMemAlloc.
        ///     Calling applications must call CoTaskMemFree to free the memory when it is no longer needed.
        /// </param>
        /// <returns>
        ///     Returns S_OK if the function succeeds. If no info tip text is available, ppwszTip is set to NULL. Otherwise,
        ///     returns a COM-defined error value.
        /// </returns>
        int IQueryInfo.GetInfoTip(QITIPF dwFlags, out string ppwszTip)
        {
            //  Do we need to get the tip on a single line?
            bool singleLine = dwFlags.HasFlag(QITIPF.QITIPF_SINGLELINE);
            ...
            try
            {
                ...
                string infoTip = GetInfo(infoType, singleLine);
