...
namespace SharpShell.Registry
{
    ...
    {
        private readonly Dictionary<Tuple<RegistryView, RegistryHive>, InMemoryRegistryKey> _rootKeys =
            new Dictionary<Tuple<RegistryView, RegistryHive>, InMemoryRegistryKey>();
        ...
        {
            Tuple<RegistryHive, string>[] hivesAndNames =
            {
            ...
            foreach (Tuple<RegistryHive, string> hn in hivesAndNames)
            foreach (RegistryView view in Enum.GetValues(typeof(RegistryView)).OfType<RegistryView>())
        ...
        public IRegistryKey OpenBaseKey(RegistryHive hKey, RegistryView view)
        {
            ...
            InMemoryRegistryKey rootKey = _rootKeys.Where(kv => kv.Key.Item1 == view && kv.Key.Item2 == hKey)
        ...
        public void AddStructure(RegistryView registryView, string structure)
        {
            ...
            Regex initialSpaceRex = new Regex(@"^([ ]+)[^ ]");

            int Spaces(string line)
            {
                Match m = initialSpaceRex.Match(line);
                return m.Success ? m.Groups[1].Length : 0;
            }

            //  Loop through the lines, building a stack of keys which we set.
            Stack<IRegistryKey> keyStack = new Stack<IRegistryKey>();
            int lineNum = 0;
            foreach (string line in structure.Split(new[] { Environment.NewLine }, StringSplitOptions.None))
            {
                lineNum++;
                ...
                int spaces = Spaces(line);
                if (spaces % 3 != 0)
                    throw new InvalidOperationException(
                        $@"Line {lineNum + 1}: Line starts with {spaces} spaces. Lines should start with spaces which are a multiple of three.");
                int depth = spaces / 3;
                ...
                {
                    //  Load the hive.
                    InMemoryRegistryKey hive = _rootKeys
                        .Where(kv => kv.Key.Item1 == registryView && kv.Value.Name == line).Select(kvp => kvp.Value)
                        .FirstOrDefault();
                    if (hive == null)
                        throw new InvalidOperationException(
                            $@"Line ${lineNum + 1}: {line} is not a known registry hive key.");
                    keyStack.Push(hive);
                    continue;
                }
                ...
                if (depth == keyStack.Count)
                {
                    ...
                    Regex rexVal = new Regex(@"^(.*) = (.*)$");
                    Match match = rexVal.Match(line.TrimStart());
                    if (match.Success)
                    {
                    ...
                    else
                    {
                        ...
                        string subkeyName = line.TrimStart();
        ...
        private static string PrintKey(IRegistryKey key, int depth)
        {
            ...
            IOrderedEnumerable<string> values = key.GetValueNames()
        ...
        public string Print(RegistryView registryView)
        {
            ...
            foreach (KeyValuePair<Tuple<RegistryView, RegistryHive>, InMemoryRegistryKey> rootKey in _rootKeys
                         .Where(rk => rk.Key.Item1 == registryView).OrderBy(k => k.Value.Name))
            {
