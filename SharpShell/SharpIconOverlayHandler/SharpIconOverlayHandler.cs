    ...
    public abstract class SharpIconOverlayHandler : SharpShellServer, IShellIconOverlayIdentifier
    {
        ...
        private string GetIconFilePath()
        {
            ...
            try
            {
                ...
                using (FileStream fileStream = new FileStream(temporaryIconOverlayFilePath, FileMode.Create))
                {
        ...
        {
            string keyName = RegistrationNameAttribute.GetRegistrationNameOrTypeName(serverType);
            Logging.Log(
                $"IconOverlayHandler: Preparing to register {registrationType} Icon Overlay Handler for type '{serverType.Name}' with key name '{keyName}'");
                       ...
                       : RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32))
            {
                ...
                using (RegistryKey overlayIdentifiers = localMachineBaseKey
                           .OpenSubKey(
                               @"Software\Microsoft\Windows\CurrentVersion\Explorer\ShellIconOverlayIdentifiers",
                               RegistryKeyPermissionCheck.ReadWriteSubTree,
                               RegistryRights.EnumerateSubKeys | RegistryRights.QueryValues |
                               RegistryRights.CreateSubKey | RegistryRights.CreateSubKey))
                {
                    ...
                    int overlayHandlersCount = overlayIdentifiers.GetSubKeyNames().Count();
                    ...
                    using (RegistryKey overlayKey = overlayIdentifiers.CreateSubKey(keyName))
                    {
        ...
        internal static void CustomUnregisterFunction(Type serverType, RegistrationType registrationType)
        {
            string keyName = RegistrationNameAttribute.GetRegistrationNameOrTypeName(serverType);
            Logging.Log(
                $"IconOverlayHandler: Preparing to unregister {registrationType} Icon Overlay Handler for type '{serverType.Name}' with key name '{keyName}'");
                       ...
                       : RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32))
            {
                ...
                using (RegistryKey overlayIdentifiers = localMachineBaseKey
                           .OpenSubKey(
                               @"Software\Microsoft\Windows\CurrentVersion\Explorer\ShellIconOverlayIdentifiers",
                               RegistryKeyPermissionCheck.ReadWriteSubTree,
                               RegistryRights.Delete | RegistryRights.EnumerateSubKeys | RegistryRights.ReadKey))
                {
                    //  If we don't have the key, we've got a problem.
                    if (overlayIdentifiers == null)
                        throw new InvalidOperationException("Cannot open the ShellIconOverlayIdentifiers key.");
        ...
        /// <param name="dwAttrib">
        ///     The object's attributes. For a complete list of file attributes and their associated flags, see
        ...
        int IShellIconOverlayIdentifier.IsMemberOf(string pwszPath, FILE_ATTRIBUTE dwAttrib)
        {
            ...
            try
            {
                ...
                bool canShowOverly = CanShowOverlay(pwszPath, dwAttrib);
            ...
            catch (Exception exception)
            {
                //  DebugLog the exception.
                LogError(
                    "IsMemberOf: An exception occured when determining whether to show the overlay for '" + pwszPath +
                    "'.", exception);
        ...
        ///     A null-terminated Unicode string that contains the fully qualified path of the file
        ///     containing the icon. The .dll, .exe, and .ico file types are all acceptable. You must set the ISIOI_ICONFILE flag
        ///     in pdwFlags if you return a file name.
        /// </param>
        /// <param name="cchMax">The size of the pwszIconFile buffer, in Unicode characters.</param>
        /// <param name="pIndex">
        ///     Pointer to an index value used to identify the icon in a file that contains multiple icons. You
        ///     must set the ISIOI_ICONINDEX flag in pdwFlags if you return an index.
        /// </param>
        /// <param name="pdwFlags">
        ///     Pointer to a bitmap that specifies the information that is being returned by the method. This
        ///     parameter can be one or both of the following values: ISIOI_ICONFILE, ISIOI_ICONINDEX.
        /// </param>
        /// <returns>
        ///     If this method succeeds, it returns S_OK. Otherwise, it returns an HRESULT error code.
        /// </returns>
        int IShellIconOverlayIdentifier.GetOverlayInfo(IntPtr pwszIconFile, int cchMax, out int pIndex,
            out ISIOI pdwFlags)
        {
            ...
            try
            {
                ...
                byte[] bytes = Encoding.Unicode.GetBytes(iconFilePath);

                if (bytes.Length + 2 < cchMax)
                {
                    for (int i = 0; i < bytes.Length; i++) Marshal.WriteByte(pwszIconFile, i, bytes[i]);
        ...
        ///     The address of a value that indicates the priority of the overlay identifier. Possible values
