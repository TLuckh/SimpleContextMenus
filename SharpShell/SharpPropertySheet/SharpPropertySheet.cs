...
namespace SharpShell.SharpPropertySheet
{
    ...
    public abstract class SharpPropertySheet : ShellExtInitServer, IShellPropSheetExt
    {
        ...
        ///     Gets the pages.
        /// </summary>
        public IEnumerable<SharpPropertyPage> Pages => propertySheetPages.Value;

        /// <summary>
        ///     Determines whether this instance can show a shell property sheet, given the specified selected file list.
        /// </summary>
        /// <returns>
        ///     <c>true</c> if this instance should show a shell property sheet for the specified file list; otherwise,
        ///     <c>false</c>.
        /// </returns>
        ...
        /// <returns>
        ///     If successful, returns a one-based index to specify the page that should be initially displayed. See Remarks for
        ///     more information.
        /// </returns>
        int IShellPropSheetExt.AddPages(IntPtr pfnAddPage, IntPtr lParam)
        {
            ...
            //  Create the bridge.
            NativeBridge.NativeBridge bridge = new NativeBridge.NativeBridge();

            //  Initialise it.
            if (bridge.Initialise() == false)
            {
                Logging.Error("Failed to initialise the NativeBridge.");
                return 0;
            }

            //  Go through each page - that has a property page.
            foreach (SharpPropertyPage page in propertySheetPages.Value.Where(p => p.Handle != IntPtr.Zero))
            {
                //  Create a property page proxy for this page.
                PropertyPageProxy proxy = new PropertyPageProxy(this, page);

                //  Using the proxy, create the property page handle.
                proxy.CreatePropertyPageHandle(bridge);
                IntPtr propertyPageHandle = proxy.HostWindowHandle;
