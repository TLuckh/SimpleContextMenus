    ...
    public static class PidlManager
    {
        ...
        public static IdList GetDesktop()
        {
            ...
            Shell32.SHGetKnownFolderIDList(KnownFolders.FOLDERID_Desktop, KNOWN_FOLDER_FLAG.KF_NO_FLAGS, IntPtr.Zero,
                out pidl);
            IdList idlist = IdList.Create(Decode(pidl));
            Shell32.ILFree(pidl);
            return idlist;
        }
        ...
        public static IntPtr IdListToPidl(IdList idList)
        {
            ...
            foreach (ShellId id in idList.Ids)
            {
            ...
            IntPtr ptr = Marshal.AllocCoTaskMem(rawBytes.Count);

            //  Copy the raw bytes.
            for (int i = 0; i < rawBytes.Count; i++) Marshal.WriteByte(ptr, i, rawBytes[i]);
        ...
        {
            List<ShellId> combined = new List<ShellId>(folderIdList.Ids);
        ...
        {
            IntPtr buffer = Marshal.AllocCoTaskMem(pidls.Length * IntPtr.Size);
