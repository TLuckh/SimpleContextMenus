...
namespace SharpShell.Diagnostics
{
    ...
    public class ExplorerConfigurationManager
    {
        ...
        /// </summary>
        private const string KeyName_AlwaysUnloadDll =
        ...
        private static bool CheckAlwaysUnloadDll()
        {
            ...
            using (IRegistryKey localMachine = registry.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Default))
            using (IRegistryKey alwaysUnloadDLLKey = localMachine.OpenSubKey(KeyName_AlwaysUnloadDll))
            {
        ...
        private static bool CheckDesktopProcess()
        {
            ...
            using (IRegistryKey currentUser = registry.OpenBaseKey(RegistryHive.CurrentUser, RegistryView.Default))
            using (IRegistryKey explorerKey = OpenExporerSubkey(currentUser, RegistryKeyPermissionCheck.ReadSubTree))
            {
                //  Do we have the value?
                object value = explorerKey.GetValue(ValueName_DesktopProcess, null);
        ...
        private void SetAlwaysUnloadDll()
        {
            ...
            if (alwaysUnloadDll)
                ...
                using (IRegistryKey localMachine =
                       registry.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Default))
                using (IRegistryKey explorerKey =
                       OpenExporerSubkey(localMachine, RegistryKeyPermissionCheck.ReadWriteSubTree))
                {
        ...
        private void SetDesktopProcess()
        {
            ...
            if (desktopProcess)
                ...
                using (IRegistryKey currentUser = registry.OpenBaseKey(RegistryHive.CurrentUser, RegistryView.Default))
                using (IRegistryKey explorerKey =
                       OpenExporerSubkey(currentUser, RegistryKeyPermissionCheck.ReadWriteSubTree))
                {
        ...
        private static IRegistryKey OpenExporerSubkey(IRegistryKey hiveKey, RegistryKeyPermissionCheck permissionCheck)
        {
            ...
            IRegistryKey explorerKey = hiveKey.OpenSubKey(KeyName_Explorer, permissionCheck);
