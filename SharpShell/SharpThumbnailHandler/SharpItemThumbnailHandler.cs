...
namespace SharpShell.SharpThumbnailHandler
{
    ...
    /// </summary>
    [Obsolete(
        "SharpItemThumbnailHandler will not work as expected from Windows Vista onwards, use SharpThumbnailHandler")]
    ...
    public abstract class SharpItemThumbnailHandler : InitializeWithItemServer, IThumbnailProvider
    {
        ...
        ///     returned bitmap should also have a 6:4 aspect ratio.
        /// </param>
        /// <param name="phbmp">
        ///     When this method returns, contains a pointer to the thumbnail image handle. The image must be a DIB
        ///     section and 32 bits per pixel. The Shell scales down the bitmap if its width or height is larger than the size
        ///     specified by cx. The Shell always respects the aspect ratio and never scales a bitmap larger than its original
        ///     size.
        /// </param>
        /// <param name="pdwAlpha">
        ///     When this method returns, contains a pointer to one of the following values from the
