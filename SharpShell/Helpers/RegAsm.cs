...
namespace SharpShell.Helpers
{
    ...
    public class RegAsm
    {
        ...
        public bool Register32(string assemblyPath, bool codebase)
        {
            ...
            string args = $@"{flags} ""{assemblyPath}""";
        ...
        private static string FindRegAsmPath(string frameworkFolder)
        {
            ...
            string searchRoot = Path.Combine(Environment.ExpandEnvironmentVariables("%WINDIR%"), @"Microsoft.Net",
                frameworkFolder);
            ...
            IOrderedEnumerable<string> sorted = candidates.OrderByDescending(s => s);
            ...
            if (path == null)
                throw new InvalidOperationException(
                    $"Failed to find regasm in '{searchRoot}'. Checked: {Environment.NewLine + string.Join(Environment.NewLine, candidates)}");
