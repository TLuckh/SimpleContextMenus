...
namespace SharpShell.Components
{
    ...
    internal static class CategoryManager
    {
        ...
        /// </summary>
        private static readonly Guid CLSID_StdComponentCategoriesMgr =
            new Guid(0x0002E005, 0x0000, 0x0000, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46);
        ...
        {
            ICatRegister catRegister = CreateCategoryRegistrationManager();
            int result = catRegister.RegisterClassImplCategories(ref clsid, 1, new[] { categoryId });
            if (result != WinError.S_OK)
                throw new ExternalException(
                    $"An exception occurred registering class {clsid} as an implementation of category {categoryId}");
        }
        ...
        {
            ICatRegister catRegister = CreateCategoryRegistrationManager();
            int result = catRegister.UnRegisterClassImplCategories(ref clsid, 1, new[] { categoryId });
            if (result != WinError.S_OK)
                throw new ExternalException(
                    $"An exception occurred registering class {clsid} as an implementation of category {categoryId}");
        }
        ...
        {
            Type type = Type.GetTypeFromCLSID(CLSID_StdComponentCategoriesMgr);
