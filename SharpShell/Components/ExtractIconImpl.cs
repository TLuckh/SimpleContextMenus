...
namespace SharpShell.Components
{
    ...
    public class ExtractIconImpl : IExtractIconA, IExtractIconW
    {
        ...
        ///     A pointer to a buffer that receives the icon location. The icon location is a null-terminated
        ///     string that identifies the file that contains the icon.
        /// </param>
        ...
        /// <returns>
        ///     Returns S_OK if the function returned a valid location, or S_FALSE if the Shell should use a default icon. If
        ///     the GIL_ASYNC flag is set in uFlags, the method can return E_PENDING to indicate that icon extraction will be
        ///     time-consuming.
        /// </returns>
        int IExtractIconA.GetIconLocation(GILInFlags uFlags, StringBuilder szIconFile, int cchMax, out int piIndex,
            out GILOutFlags pwFlags)
        {
        ...
        ///     time-consuming.
        /// </returns>
        int IExtractIconW.GetIconLocation(GILInFlags uFlags, StringBuilder szIconFile, int cchMax, out int piIndex,
            out GILOutFlags pwFlags)
        {
            return GetIconLocation(uFlags, out piIndex, out pwFlags);
        }
        ...
        ///     A pointer to an HICON value that receives the handle to the large icon. This parameter may be
        ///     NULL.
        /// </param>
        /// <param name="phiconSmall">
        ///     A pointer to an HICON value that receives the handle to the small icon. This parameter may be
        ///     NULL.
        /// </param>
        /// <param name="nIconSize">
        ///     The desired size of the icon, in pixels. The low word contains the size of the large icon, and
        ///     the high word contains the size of the small icon. The size specified can be the width or height. The width of an
        ///     icon always equals its height.
        /// </param>
        /// <returns>
        ///     Returns S_OK if the function extracted the icon, or S_FALSE if the calling application should extract the icon.
        /// </returns>
        int IExtractIconA.Extract(string pszFile, uint nIconIndex, out IntPtr phiconLarge, out IntPtr phiconSmall,
            uint nIconSize)
        {
        ...
        ///     Returns S_OK if the function extracted the icon, or S_FALSE if the calling application should extract the icon.
        /// </returns>
        int IExtractIconW.Extract(string pszFile, uint nIconIndex, out IntPtr phiconLarge, out IntPtr phiconSmall,
            uint nIconSize)
        {
            return Extract(out phiconLarge, out phiconSmall, nIconSize);
        }
        ...
        private int Extract(out IntPtr phiconLarge, out IntPtr phiconSmall, uint nIconSize)
        {
            ...
            uint iconSizeSmall = (uint)User32.HighWord((int)nIconSize);
            uint iconSizeLarge = (uint)User32.LowWord((int)nIconSize);
            ...
            {
                Logging.Error(
                    $"An exception occured extracting icons at, small size '{iconSizeSmall}', large size {iconSizeLarge}.",
                    exception);
